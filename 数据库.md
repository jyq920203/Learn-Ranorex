...
# 如何创建数据库连接池？
我使用的方法是建立了三个独立的Code Module，一个文件创建数据库连接池，一个文件进行查询操作，一个文件进行进行数据库连接池的销毁，这里创建了个一个公共变量public static OracleConnection con;这样就可以在不同的文件中使用了，因为连接池的创建需要在用例的Set up阶段进行使用。数据库查询在Test Case中，数据库连接池的销毁需要在Tear Down的时候使用，分布在不同的文件中，所以需要有一个全局的变量让他们公用一个连接池。
    ...
    public class ConnectP : ITestModule
    {
        ...
        void Connectp()
        {
		    ...
		    con.ConnectionString = 
		      "User Id=trade;Password=trade;Data Source=HSPB_TEST_NEW;" + 
